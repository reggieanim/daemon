...
// FileHandler defines an interface for file operations.
type FileHandler interface {
	Open(name string) (io.ReadCloser, error)
	ReadFile(name string) ([]byte, error)
}

// OSFileHandler implements FileHandler using the actual OS functions.
type OSFileHandler struct{}

func (h *OSFileHandler) Open(name string) (io.ReadCloser, error) {
	return os.Open(name)
}
